...
void* reinaThread(void* parametro)
{
  ...
  int opcion;
  int hormiga_actual;
  int goles_totales_equipo_reina;
  int gol;
...
*/
  ...
  while(goles_totales_equipo_reina != 5)
  {
    lockMutex(&mutex);
    ...
    switch(msg.intEvento)
    {
      ...
      */
      ...
      case EVT_PATEAR:
        printf("va patear el equipo Reina");
        gol = obtenerNumeroAleatorio(JUNTAR_MIN, JUNTAR_MAX);
        if(gol == 1)
        {
          goles_totales_equipo_reina = goles_totales_equipo_reina + 1;
        ...
        /*usleep(INTERVALO_REINA_MS * 1000);*//*espera entre disparos*/
        /*Pasa a Jugador MISMO NUMERO del equipo  hormiga*/
        lockMutex(&mutex);
        enviarMensaje(datosThread->idColaMensajes, MSG_HORMIGA + hormiga_actual, MSG_REINA, EVT_PATEAR, "");
        unlockMutex(&mutex);
        break;
    ...
    {
      lockMutex(&mutex);
      enviarMensaje(idColaMensajes, MSG_HORMIGA + i, MSG_REINA, EVT_FIN_HORMIGA, "");
      unlockMutex(&mutex);
    }
