...
void replazarLineaArchivo(char *mensaje , int delete_line , char *nombreArchivo) {
    FILE *arcAux;
    //char filechar[40] = nombreArchivo;
    char c;
    //int delete_line; 
	int temp = 1;
	...
	*/
	...
	if (abrirArchivo( nombreArchivo, "w" ) == 0) {
		c = getc(arc);
		while (c != EOF)
		{
			if (c == 'n')
		...
		rename("replica.c", nombreArchivo);
		//arc = fopen(nombreArchivo, "r");
...
int leerLineaArchivo( int linea, char *nombreArchivo ) {
	int temp = 0;
	char c;
	char texto[200]

	if (abrirArchivo( nombreArchivo, "r" ) == 0) {
		c = getc(arc);
		while (c != EOF){
			if (c == 'n'){ //cuenta las lineas
				temp++;
			}
        	//till the line to be deleted comes,copy the content to other
			if (temp != linea)
