...
int main(int argc, char *argv[])
{
  ...
  for (i = 0; i < CANT_COMIDAS; i++)                /*cant_comidas = cant_kartings*/
  {
    ...
    {
      printf("Karting %d\n", i + 1);
      solicitarCadena(comidas[i].descripcion, "Ingrese nombre del karting:");
    }
    puts("");
    comidas[i].precio = 0;          /*precio = irrelebante en este ejercicio la dejo para no arriesgarme a romper nada*/
    puts("");
  ...
  while(1)
  {
    for (i = 0; i < CANT_COMIDAS; i++)                                       /*cant_comidas = cant_kartings*/
    {
      ...
      comidas[i].precio = comidas[i].total;	
    ...
    if ( (comidas[0].total >= (MAX_PEDIDOS)) || (comidas[1].total >= (MAX_PEDIDOS)) || (comidas[2].total >= (MAX_PEDIDOS)))
    {
      ...
      if ( comidas[0].total >= (MAX_PEDIDOS)){
	puts("Karting 1 llego a la meta");
      }
      if ( comidas[1].total >= (MAX_PEDIDOS)){
	puts("Karting 2 llego a la meta");
      }
      if ( comidas[2].total >= (MAX_PEDIDOS)){
	puts("Karting 3 llego a la meta");
      }
      break;
    }
  }
